# -*- coding: utf-8 -*-
"""
Created on Wed Jan 15 20:59:33 2020
...
# %%
        ...
        for key in LAWS:
            ...
            for x2_idx, x2 in enumerate(X2):
                ...
                init_pose = pf.GeckoBotPose(
                        *model.set_initial_pose(ref2[0][0], eps0-30, (0, 0),
                                                len_leg=len_leg, len_tor=len_tor))

                # sim one fast cycle  -> drop first cycle
                init_ref = [ref2[int(res-1)], ref2[-1]]
                init_gait = pf.predict_gait(init_ref, init_pose, weight, (len_leg, len_tor))
                init_pose = init_gait.poses[-1]
    ...
    #            plt.legend(loc='center left', bbox_to_anchor=(1, .5))
                ...
                kwargs = {'extra_axis_parameters':
                      {'anchor=origin'}}
                save_utils.save_plt_as_tikz('Out/analytic_model9/'+name, **kwargs)
#
...
#                
            ...
            # %%
#                fig.clear()
#                pf.save_animation(gait.animate(), '../../Out/analytic_model9/gait.mp4')

            save_utils.save_data(RESULT, 'sim_result.h5')
            save_utils.save_data(REF, 'sim_ref.h5')
            save_utils.save_data(TIME, 'sim_time.h5')

            # %% POSES
            gait_ = pf.GeckoBotGait()
            gait_.append_pose(gait.poses[1])  # init pose
            gait_.append_pose(gait.poses[int((res))])  # midpose
            gait_.append_pose(gait.poses[int((res+1))])  # midpose + 1
            gait_.append_pose(gait.poses[-1])  # end pose
            gait_.plot_gait()
            gait_.plot_orientation(length=5)
            gait_.save_as_tikz('/Out/analytic_model9/gait')

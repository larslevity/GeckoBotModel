...
x1_fit = np.arange(60, 100.1, 10)
x2_fit = np.arange(-.5, .51, .1)
...
#    plt.plot(data2['x2'][jdx*idx:(jdx+1)*idx], data2['x3'][jdx*idx:(jdx+1)*idx], ':', color=col2)
    ...
    x3_fit = [x3fun(x1_fit[jdx], x2_) for x2_ in x2_fit]
    plt.plot(x2_fit, x3_fit, ':', color=col2)
...
#    plt.plot(data2['x2'][jdx*idx:(jdx+1)*idx], data2['x4'][jdx*idx:(jdx+1)*idx], ':', color=col1)
    ...
    x4_fit = [x4fun(x1_fit[jdx], x2_) for x2_ in x2_fit]
    plt.plot(x2_fit, x4_fit, ':', color=col1)




plt.legend(['x3 for x1={}'.format(int(60 + 10*x)) for x in range(lx1)]
            + ['x3_fit for x1={}'.format(int(60 + 10*x)) for x in range(lx1)]
            + ['x4 for x1={}'.format(int(60 + 10*x)) for x in range(lx1)]
            + ['x4_fit for x1={}'.format(int(60 + 10*x)) for x in range(lx1)])
...
fig.savefig('Out/analytic_model3/X3X4.png', dpi=300)

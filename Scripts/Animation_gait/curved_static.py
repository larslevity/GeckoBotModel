...
xpos5, ypos5 = gait.poses[idx5].get_m1_pos()
xpos6, ypos6 = gait.poses[idx6].get_m1_pos()
...
\\path[use as bounding box](-20,-18) rectangle (15,18);
""" % 0
    ...
    init5 = '\\path[fill=gray!50] (%f,%f)circle(1);\n' % (round(-xpos+xpos5, 4), round(-ypos+ypos5, 4))
    init5_ = '\\path[fill=gray!50] (%f,%f)circle(1);\n' % (round(-xpos+xpos5, 4), round(-ypos-ypos5, 4))
    init6 = '\\path[fill=gray!50] (%f,%f)circle(1);\n' % (round(-xpos+xpos6, 4), round(-ypos+ypos6, 4))
    init6_ = '\\path[fill=gray!50] (%f,%f)circle(1);\n' % (round(-xpos+xpos6, 4), round(-ypos-ypos6, 4))
    ani_str += (header 
#                + init1 +  init3 + init5
                + init + init2 + init4 + init6
#                + init1_ + init2_ + init3_ + init4_ + init5_ + init6_
...
filename = 'Out/curved_gait_static_%s_%s.tex' % (round(Q1), str(round(Q2,1)).replace('.', '_'))
